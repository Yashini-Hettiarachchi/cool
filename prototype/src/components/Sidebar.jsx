...
const NAV = {
  ...
  technician: [
    { label: 'Work', items: [
      { to: '/technician', icon: 'jobs', name: 'My Jobs', end: true },
      { to: '/technician/search', icon: 'search', name: 'Find Job' },
    ] },
  ],
};
...
export default function Sidebar({ open, onNavigate }) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    const refresh = () => jobsApi.getDashboardMetrics()
      .then(({ pendingApproval }) => { if (alive) setPending(pendingApproval || 0); })
      .catch(() => {});
    refresh();
